    ...
    <div>
        <div class="max-w-md mx-auto mt-10 p-6 bg-white rounded-lg shadow-lg" :class="{ 'bg-yellow-50': isEditing }">
            ...
            <form @submit.prevent="handleSubmit" class="flex flex-col space-y-4">
                ...
                <input type="text" v-model="description" placeholder="Descripción del gasto" required
                    :disabled="loading"
                    class="border border-gray-500 rounded p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" />
        ...
        <!-- Add Confirm Dialog -->
        <ConfirmDialog :show="showConfirmDialog" title="Confirmar modificación"
            message="¿Está seguro que desea modificar este gasto?" confirm-text="Actualizar" cancel-text="Cancelar"
            confirm-button-class="bg-yellow-500 hover:bg-yellow-600" @confirm="confirmUpdate" @cancel="cancelConfirm" />

        <ConfirmDialog :show="showConfirmDialog" title="Confirmar modificación"
            message="¿Está seguro que desea modificar este gasto?" @confirm="confirmUpdate" @cancel="cancelConfirm" />
    </div>
</template>
...
const handleSubmit = () => {
    ...
    if (isEditing.value) {
        ...
        pendingUpdate.value = expenseData
        showConfirmDialog.value = true
    } else {
        // Direct emission for new expenses
...
const confirmUpdate = () => {
    emit('update-expense', props.expenseToEdit.id, pendingUpdate.value)
